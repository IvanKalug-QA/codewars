...
def left(strng: str, i: str | int = 1) -> str:
    if isinstance(i, str):
        pos = strng.find(i)
        if pos == -1:
            return ''
    ...
    if i < 0:
        n = abs(i)
        return strng[:len(strng) - n] if n < len(strng) else ''
    if i >= len(strng):
        return strng
    return strng[:i]

def right(strng: str, i: str | int = 1) -> str:
    if isinstance(i, str):
        pos = strng.rfind(i)
        if pos == -1:
            return ''
        return strng[pos + len(i):]
    
    if i == 0:
        return ''
    if i < 0:
        n = abs(i)
        return strng[n:] if n < len(strng) else ''
    if i >= len(strng):
        return strng
    return strng[-i:]