...
# Output
# The function should return a new string representing the seating arrangement, with the chosen ideal empty seat marked as occupied (1).
# If there are no empty seats (0) available, or if all available seats are unacceptable (have 2 threats), the function must return None.
# Examples
# "1000100" -> "1000101"
# "000" -> "100"
# "000 01" -> "100 01"
...
def introverted_seat(seats: str) -> str | None:
    ...
    for i, char in enumerate(seats):
        ...
        if i > 0 and seats[i-1] != ' ':
            left_threat = 1
        if i < len(seats)-1 and seats[i+1] != ' ':
            right_threat = 1
        
        total_threats = left_threat + right_threat
        if total_threats >= 2:
            continue
        if total_threats < best_threat_count:
            best_threat_count = total_threats
            best_seat_index = i
    if best_seat_index == -1:
        return None
    result = list(seats)
    result[best_seat_index] = '1'
    return ''.join(result)