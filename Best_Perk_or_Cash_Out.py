...
def pick(preferred, blacklisted, options):
    ...
    for i, (skill, percentage) in enumerate(options):
        if skill in preferred and skill not in blacklisted:
            preferred_options.append((percentage, i))
    if preferred_options:
        max_percentage, index = max(preferred_options)
        return chr(ord('A') + index)
    neutral_options = []
    for i, (skill, percentage) in enumerate(options):
        if skill not in blacklisted and skill not in preferred:
            neutral_options.append((percentage, i))
    if neutral_options:
        max_percentage, index = max(neutral_options)
        return chr(ord('A') + index)
    return "D"