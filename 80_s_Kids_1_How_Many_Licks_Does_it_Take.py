...
def total_licks(env):
    base_licks = 252
    total = base_licks
    max_challenge = {"name": None, "value": float('-inf')}
    for condit, value in env.items():
        total += value
        if value > 0 and value > max_challenge["value"]:
            max_challenge = {"name": condit, "value": value}
    result = f"It took {total} licks to get to the tootsie roll center of a tootsie pop."

    if max_challenge["name"]:
        result += f" The toughest challenge was {max_challenge['name']}."
    return result