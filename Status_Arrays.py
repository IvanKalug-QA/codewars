...
def status(nums):
    ...
    for i, num in enumerate(nums):
        count_less = sum(1 for x in nums if x < num)
        status_value = i + count_less
        elements_with_status.append((num, i, status_value))
    
    elements_with_status.sort(key=lambda x: (x[2], x[1]))
    
    return [element[0] for element in elements_with_status]