...
def drink_water(bottle_height, bottle_radius, water_height, crow_mouth, little_stones):
    if water_height + crow_mouth >= bottle_height:
        return []
    needed_rise = bottle_height - water_height - crow_mouth
    needed_volume = needed_rise * math.pi * (bottle_radius ** 2)
    total_volume = 0
    for i, stone in enumerate(little_stones):
        total_volume += stone
        if total_volume >= needed_volume:
            return little_stones[:i+1]
