...
def is_sator_square(tablet):
    ...
    rows_lr = [''.join(row) for row in tablet]
    
    rows_rl = [''.join(row[::-1]) for row in tablet]
    
    cols_td = [''.join(tablet[i][j] for i in range(N)) for j in range(N)]
    
    cols_bu = [''.join(tablet[N-1-i][j] for i in range(N)) for j in range(N)]
    
    return sorted(rows_lr) == sorted(rows_rl) == sorted(cols_td) == sorted(cols_bu)