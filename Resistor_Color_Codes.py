...
def decode_resistor_colors(bands):
    ...
    first = colors[bands_list[0]]
    second = colors[bands_list[1]]
    multiplier = colors[bands_list[2]]
    resistance = (first * 10 + second) * (10 ** multiplier)
    if len(bands_list) == 4:
        if bands_list[3] == 'gold':
            tolerance = 5
        else:
            tolerance = 10
    else:
    ...
    if resistance < 1000:
        formatted_resistance = f"{resistance}"
    elif resistance < 1000000:
        value = resistance / 1000
        if value.is_integer():
            formatted_resistance = f"{int(value)}k"
        else:
            formatted_resistance = f"{value:.1f}k".rstrip('0').rstrip('.')
    else:
        value = resistance / 1000000
        if value.is_integer():
            formatted_resistance = f"{int(value)}M"
        else:
            formatted_resistance = f"{value:.1f}M".rstrip('0').rstrip('.')
    
    return f"{formatted_resistance} ohms, {tolerance}%"