...
def unscramble(scramble):
    ...
    if not hasattr(unscramble, 'cache'):
        unscramble.cache = {}
        for word in word_list:
            sorted_word = ''.join(sorted(word))
            if sorted_word not in unscramble.cache:
                unscramble.cache[sorted_word] = []
            unscramble.cache[sorted_word].append(word)
    
    sorted_scramble = ''.join(sorted(scramble))
    return unscramble.cache.get(sorted_scramble, [])