# Task
# EvilCode is a game similar to Codewars.
# You have to solve programming tasks as quickly as possible. However,
# unlike Codewars, EvilCode awards you with a medal,
# depending on the time you took to solve the task.
#
# To get a medal, your time must be (strictly) inferior to the time corresponding to the medal.
# You can be awarded "Gold", "Silver" or "Bronze" medal, or "None" medal at all.
# Only one medal (the best achieved) is awarded.
#
# You are given the time achieved for the task and the time corresponding to each medal.
...
def evil_code_medal(user_time, gold, silver, bronze):
    print(user_time, gold, silver, bronze)
    if user_time < gold and user_time < silver:
        return 'Gold'
    if user_time < silver and user_time >= gold:
        return 'Silver'
    if user_time < bronze:
        return 'Bronze'
    return 'None'