...
def reach_destination(distance, speed):
    ...
    rounded = round(time * 2) / 2  # nearest half hour
    if rounded.is_integer():
        hours = int(rounded)
        return f'The train will be there in {hours} hour.' if hours == 1 else f'The train will be there in {hours} hours.'
    else:
        return f'The train will be there in {rounded} hours.'